      ...
      {chat?.products && (
        <div className="bg-amber-50 border-b border-amber-100 px-4 py-2 flex items-center gap-3">
          {chat.products.images?.[0] && (
            <img src={chat.products.images[0]} className="w-10 h-10 rounded object-cover" />
          )}
          <div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.is_admin ? "justify-start" : "justify-end"}`}>
            {msg.is_admin && (
              <div className="w-7 h-7 rounded-full bg-gray-900 text-white flex items-center justify-center text-xs font-bold mr-2 flex-shrink-0 self-end">K</div>
            )}
            <div className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm ${
              msg.is_admin ? "bg-white text-gray-800 shadow-sm border rounded-bl-sm" : "bg-gray-900 text-white rounded-br-sm"
            }`}>
              {msg.content}
              <p className={`text-xs mt-1 ${msg.is_admin ? "text-gray-400" : "text-gray-300"}`}>
