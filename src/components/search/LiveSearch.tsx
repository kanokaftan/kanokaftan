...
export function LiveSearch({ className, placeholder = "Search products...", autoFocus }: LiveSearchProps) {
  ...
  const { data: productResults = [], isLoading, isFetching } = useSearchProducts(query, isOpen);
    ...
    <div ref={containerRef} className={cn("relative", className)}>
      <form onSubmit={handleSubmit}>
        <div className="relative">
          ...
          />
          {query && (
            <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7" onClick={() => setQuery("")}>
              <X className="h-4 w-4" />
            </Button>
          )}
        </div>
      ...
      {showResults && (
        <div className="absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden rounded-xl border bg-popover shadow-lg animate-in fade-in-0 slide-in-from-top-2 max-h-[70vh] overflow-y-auto">
          ...
          {showRecent && !showLoading && (
            <div className="p-2">
              <p className="px-2 py-1 text-xs font-medium text-muted-foreground flex items-center gap-1">
                <TrendingUp className="h-3 w-3" /> Recently Viewed
              </p>
              {recentProducts.slice(0, 5).map((product) => (
                ...
                >
                  {product.imageUrl && <img src={product.imageUrl} alt="" className="h-10 w-10 rounded-md object-cover" />}
          ...
          {!showRecent && !showLoading && productResults.length > 0 && (
            <div className="p-2 space-y-1">
              {productResults.map((result) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-lg p-2 text-left transition-colors hover:bg-muted"
                >
                  {result.imageUrl && <img src={result.imageUrl} alt="" className="h-10 w-10 rounded-md object-cover" />}
                  <div className="flex-1 min-w-0">
              ...
              <button onClick={handleSubmit} className="mt-1 w-full rounded-lg bg-muted p-2 text-center text-sm text-muted-foreground hover:bg-muted/80">
                View all results for "{query}"
              </button>
            </div>
          )}

          {!showRecent && !showLoading && query.length >= 2 && productResults.length === 0 && (
